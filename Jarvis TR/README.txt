# 🤖 Jarvis Mark X — Local Yapay Zeka Asistanı

Jarvis Mark X, **Python** ile sıfırdan geliştirilmiş, Jarvis’ten ilham alan **tamamen local** bir masaüstü yapay zeka asistanıdır.

Sesli komutlarla bilgisayar kontrolü sağlar ve intent → action mantığıyla çalışır.

---

## 🚀 Özellikler
- Uygulama açma
- Mesaj gönderme (WhatsApp)
- İnternet araması & hava durumu
- Geçici & kalıcı hafıza
- Tamamen local çalışma
- İyi hafıza ile genel sohbet

## 🎥 Tutorial
Bu proje bir **video anlatım serisinin** parçasıdır.  
Kod yapısını ve güvenli geliştirmeyi anlamak için videoyu izlemeniz önerilir.

---

## 🔐 Güvenlik Uyarısı
- Program **%100 local çalışır**
- Varsayılan olarak kimse erişemez
- Jarvis sadece **senin yazdığın kodları** çalıştırır

⚠️ Bilinmeyen kodları yapıştırmayın ve sisteme tam yetki vermeyin.

---

## ⚠️ Kullanım Uyarısı
- **Ticari kullanım yasaktır**
- Satılamaz, pazarlanamaz
- Kendi yapmış gibi sosyal medyada paylaşılamaz

Kişisel ve eğitim amaçlı kullanabilirsiniz.

---

## 📲 Takip & İletişim
Instagram: https://www.instagram.com/fatihmakes/  
TikTok: https://www.tiktok.com/@fatihmakes  
YouTube:

Sorunlar için yazabilirsiniz.

⭐ Takip etmeyi unutmayın!
